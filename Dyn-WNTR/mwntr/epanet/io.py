...
import six
import mwntr
from mwntr.epanet.exceptions import ENKeyError, ENSyntaxError, ENValueError, EpanetException
import mwntr.network
from mwntr.network.base import Link
from mwntr.network.controls import (AndCondition, Comparison, Control,
                                   ControlAction, OrCondition, Rule,
                                   SimTimeCondition, TimeOfDayCondition,
                                   ValueCondition, _ControlType)
from mwntr.network.elements import Junction, Pipe, Pump, Reservoir, Tank, Valve
from mwntr.network.model import (Curve, Demands, LinkStatus, Pattern, Source,
                                WaterNetworkModel)
from mwntr.network.options import Options
...
class InpFile(object):
    ...
    def _write_title(self, f, wn):
        if wn.name is not None:
            ...
            f.write('; WNTR: {}\n; Created: {:%Y-%m-%d %H:%M:%S}\n'.format(mwntr.__version__, datetime.datetime.now()).encode(sys_default_enc))
...
#            assert(len(current) == 2), ("Error reading [STATUS] block, Check format.")
            ...
            if (current[1].upper() == 'OPEN' or
                ...
                if isinstance(link, mwntr.network.Valve):
        ...
        for line in self._if_clauses:
            ...
            if words[1].upper() == 'SYSTEM':
                ...
                if attr.lower() in ['demand']:
                    ...
                    if isinstance(link, mwntr.network.Pump):
                        value = value
                    elif isinstance(link, mwntr.network.Valve):
...
class BinFile(object):
    ...
    def __init__(self, result_types=None, network=False, energy=False, statistics=False,
        ...
        self.results = mwntr.sim.SimulationResults()
...
def _read_control_line(line, wn, flow_units, control_name):
    ...
    if status == 'OPEN' or status == 'OPENED' or status == 'CLOSED' or status == 'ACTIVE':
        ...
        action_obj = mwntr.network.ControlAction(link, 'status', setting)
    else:
        if isinstance(link, mwntr.network.Pump):
            action_obj = mwntr.network.ControlAction(link, 'base_speed', float(current[2]))
        elif isinstance(link, mwntr.network.Valve):
            ...
            action_obj = mwntr.network.ControlAction(link, 'setting', setting)
        else:
