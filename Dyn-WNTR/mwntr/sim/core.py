...
from mwntr.sim.solvers import NewtonSolver, SolverStatus
...
from mwntr.utils.ordered_set import OrderedSet
from mwntr.network import Junction, Pipe, Valve, Pump, Tank, Reservoir, LinkStatus, WaterNetworkModel, Link
from mwntr.sim.network_isolation import check_for_isolated_junctions, get_long_size
from mwntr.sim.aml.aml import VarDict, ParamDict
from mwntr.sim.aml.expr import Var, Param
from mwntr.network.controls import (AndCondition, Comparison, Control, ControlAction,
...
class _Diagnostics(object): # pragma: no cover
    ...
    def compare_link_sol(self):
        ...
        if isinstance(start_node, mwntr.network.Junction):
        ...
        if isinstance(end_node, mwntr.network.Junction):
    ...
    def store_var_values_in_network(self):
        # self.mode = self._wn.options.hydraulic.demand_model
        mwntr.sim.hydraulics.store_results_in_network(self.wn, self.model) #, self.mode)
...
class _ValveSourceChecker(Observer):
    ...
    def update(self, action: BaseControlAction):
        ...
        if val != self._previous_values[(obj, attr)]:
            self._needs_compute = True
            if val == mwntr.network.LinkStatus.Closed:
...
class WNTRSimulator(WaterNetworkSimulator):
    ...
    def run_sim(self, solver=NewtonSolver, backup_solver=None, solver_options=None,
        ...
        ----------
        solver: object
            :py:class:`~mwntr.sim.solvers.NewtonSolver` or Scipy solver
        backup_solver: object
            :py:class:`~mwntr.sim.solvers.NewtonSolver` or Scipy solver
        solver_options: dict
            See :py:class:`~mwntr.sim.solvers.NewtonSolver` for possible options
        ...
        while True:
            ...
            if solver_status == 0:
                ...
                results.error_code = mwntr.sim.results.ResultsStatus.error
            ...
            if isinstance(self._report_timestep, (float, int)):
                if self._wn.sim_time % self._report_timestep == 0:
                    mwntr.sim.hydraulics.save_results(self._wn, node_res, link_res)
        ...
        mwntr.sim.hydraulics.get_results(self._wn, results, node_res, link_res)
    ...
    def _initialize_internal_graph(self):
        ...
        for link_name, link in itertools.chain(self._wn.pipes(), self._wn.pumps(), self._wn.valves()):
            ...
            if link.status == mwntr.network.LinkStatus.Closed:
                vals.append(0)
                vals.append(0)
            else:
                vals.append(1)
    ...
    def _update_internal_graph(self):
        ...
        for obj, attr in self._change_tracker.get_changes(ref_point='graph'):
            if 'status' == attr:
                if obj.status == mwntr.network.LinkStatus.Closed:
    ...
    def _get_isolated_junctions_and_links(self):
        ...
        mwntr.sim.hydraulics.update_model_for_isolated_junctions_and_links(self._model, self._wn, self._model_updater,
