...
import mwntr.epanet
from mwntr.epanet.util import FlowUnits
import mwntr.network.model
from mwntr.gis.network import WaterNetworkGIS
try:
...
def from_dict(d: dict, append=None):
    ...
    """
    from mwntr.epanet.io import _read_control_line, _EpanetRule
...
def write_inpfile(wn, filename: str, units=None, version: float = 2.2, 
    ...
    if wn._inpfile is None:
        ...
        wn._inpfile = mwntr.epanet.InpFile()
    if units is None:
